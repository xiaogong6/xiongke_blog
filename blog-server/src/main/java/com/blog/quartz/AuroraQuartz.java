...
public class AuroraQuartz {
    ...
    public void baiduSeo() {
        List<Integer> ids = articleService.list().stream().map(Article::getId).toList();
    ...
    public void importSwagger() {
        ...
        List<Integer> resourceIds = resourceService.list().stream().map(Resource::getId).toList();
